...
module.exports = function PDFPuzzleDoc({ puzzle }) {
  ...
  const wantLength = (totalClueHeight + ((columnCount - 1) * puzzle.grid.length * lnsPerSqr)) / columnCount;
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.gridrow}>
          <View style={[styles.column, (columnCount === 4 ? styles.column20 : styles.column33)]}>
          ...
          <View style={[styles.column, (columnCount === 4 ? styles.column80 : styles.column66)]}>
            <PrintGrid puzzle={puzzle} styles={styles} />
            <View style={{flexDirection: 'row'}}>
              <View style={[styles.column, (columnCount === 4 ? styles.column33 : styles.column50)]}>
                {[...Array(allClues.length)].map((key, i) => {
                  if (usedClueHeight < (wantLength + ((wantLength - (puzzle.grid.length * lnsPerSqr)) * 1))) {
                    let curClue = allClues.shift();
                    usedClueHeight += clueHeight(curClue[2]);
                    return (
                      <PrintClue key={curClue[0]+curClue[1]} num={curClue[1]} clue={curClue[2]} />
              ...
              <View style={[styles.column, (columnCount === 4 ? styles.column33 : styles.column50)]}>
              {[...Array(allClues.length)].map((key, i) => {
                if (usedClueHeight < (wantLength + ((wantLength - (puzzle.grid.length * lnsPerSqr)) * 2))) {
                  let curClue = allClues.shift();
                  usedClueHeight += clueHeight(curClue[2]);
                  return (
                    <PrintClue key={curClue[0]+curClue[1]} num={curClue[1]} clue={curClue[2]} />
              ...
              <View style={[styles.column, (columnCount === 4 ? styles.column33 : styles.column0)]}>
              {[...Array(allClues.length)].map((key, i) => {
                if (usedClueHeight < (wantLength + ((wantLength - (puzzle.grid.length * lnsPerSqr)) * 3))) {
                  let curClue = allClues.shift();
                  usedClueHeight += clueHeight(curClue[2]);
                  return (
                    <PrintClue key={curClue[0]+curClue[1]} num={curClue[1]} clue={curClue[2]} />
